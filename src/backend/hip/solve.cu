...
// hipblasStatus_t cublas<>getrsBatched( hipblasHandle_t handle,
...
struct getrsBatched_func_def_t {
    typedef hipblasStatus_t (*getrsBatched_func_def)(hipblasHandle_t,
...
// hipblasStatus_t cublas<>getrfBatched(hipblasHandle_t handle,
//                                     int n,
//                                     float *A[],
//                                     int lda,
//                                     int *P,
...
struct getrfBatched_func_def_t {
    typedef hipblasStatus_t (*getrfBatched_func_def)(hipblasHandle_t, int, T **,
                                                    int, int *, int *, int);
};
...
// hipsolverStatus_t cusolverDn<>getrs(
...
struct getrs_func_def_t {
    typedef hipsolverStatus_t (*getrs_func_def)(hipsolverHandle_t,
                                               hipblasOperation_t, int, int,
...
// hipsolverStatus_t cusolverDn<>geqrf_bufferSize(
//        hipsolverHandle_t handle,
//        int m, int n,
//        <> *A,
//        int lda,
//        int *Lwork );
//
// hipsolverStatus_t cusolverDn<>geqrf(
//        hipsolverHandle_t handle,
//        int m, int n,
//        <> *A, int lda,
//        <> *TAU,
...
//
// hipsolverStatus_t cusolverDn<>mqr(
//        hipsolverHandle_t handle,
...
struct geqrf_solve_func_def_t {
    typedef hipsolverStatus_t (*geqrf_solve_func_def)(hipsolverHandle_t, int,
                                                     int, T *, int, T *, T *,
                                                     int, int *);
};
...
struct geqrf_solve_buf_func_def_t {
    typedef hipsolverStatus_t (*geqrf_solve_buf_func_def)(hipsolverHandle_t,
                                                         int, int, T *, int,
                                                         int *);
};
...
struct mqr_solve_func_def_t {
    typedef hipsolverStatus_t (*mqr_solve_func_def)(
        hipsolverHandle_t, hipblasSideMode_t, hipblasOperation_t, int, int, int,
        const T *, int, const T *, T *, int, T *, int, int *);
};

template<typename T>
struct mqr_solve_buf_func_def_t {
    typedef hipsolverStatus_t (*mqr_solve_buf_func_def)(
	hipsolverHandle_t, hipblasSideMode_t, hipblasOperation_t, int, int, int,
...
#define MQR_FUNC(FUNC, TYPE, PREFIX)                                            \
    ...
    typename FUNC##_solve_buf_func_def_t<TYPE>::FUNC##_solve_buf_func_def       \
        ...
        return (FUNC##_solve_buf_func_def_t<TYPE>::FUNC##_solve_buf_func_def) & \
               hipsolverDn##PREFIX##_bufferSize;                                 \
    }
...
Array<T> generalSolveBatched(const Array<T> &a, const Array<T> &b) {
    ...
    CUDA_CHECK(hipMemcpyAsync(aBatched_device_ptrs, aBatched_host_ptrs, bytes,
    ...
    CUDA_CHECK(hipMemcpyAsync(bBatched_device_ptrs, bBatched_host_ptrs, bytes,
                               hipMemcpyHostToDevice,
                               getStream(getActiveDeviceId())));
    ...
    CUBLAS_CHECK(getrsBatched_func<T>()(
        blasHandle(), HIPBLAS_OP_N, N, NRHS, (const T **)aBatched_device_ptrs,
