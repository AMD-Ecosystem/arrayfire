...
static void bcast_first_launcher(Param<To> out, Param<To> tmp, Param<int> tlid,
    ...
    // The shared source also defines scanbykey_first_nonfinal/final, whose
    // bodies use the non-dependent THREADS_PER_BLOCK / DIMX; clang/hipRTC parses
    // them even for the bcast instantiation, so both -D must be present here too.
