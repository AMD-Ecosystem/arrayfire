...
void lookup(Param<in_t> out, CParam<in_t> in, CParam<idx_t> indices, int nDims,
    ...
    if (dim == 0 && nDims == 1 && dim == vDim) {
        ...
        // lookup.cuh also defines lookup1D, whose body uses the non-dependent
        // THREADS / THRD_LOAD; clang/hipRTC parses it even though only lookupND
        // is instantiated, so both -D must be present for this compile too.
