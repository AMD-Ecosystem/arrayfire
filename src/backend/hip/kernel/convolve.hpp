        ...
        for (int b2 = 0; b2 < filt.dims[2]; ++b2) {
            ...
            for (int b1 = 0; b1 < filt.dims[1]; ++b1) {
                ...
                convolve1.copyToReadOnly(constMemPtr,
                                         reinterpret_cast<CUdeviceptr>(const_cast<void *>(static_cast<const void *>(fptr))),
                                         filterSize);
...
void conv2Helper(const conv_kparam_t& p, Param<T> out, CParam<T> sig,
    ...
    convolve2.copyToReadOnly(constMemPtr, reinterpret_cast<CUdeviceptr>(const_cast<void *>(static_cast<const void *>(fptr))),
                             f0 * f1 * sizeof(aT));
...
void convolve_3d(conv_kparam_t& p, Param<T> out, CParam<T> sig, CParam<aT> filt,
    ...
    for (int b3 = 0; b3 < filt.dims[3]; ++b3) {
        ...
        convolve3.copyToReadOnly(
            constMemPtr, reinterpret_cast<CUdeviceptr>(const_cast<void *>(static_cast<const void *>(fptr))), filterSize);
...
void convolve2(Param<T> out, CParam<T> signal, CParam<aT> filter, int conv_dim,
    ...
    // CUdeviceptr is an integer on CUDA but void* on HIP (hipDeviceptr_t), so a
    // reinterpret_cast from a const pointer drops the const qualifier on HIP;
    // cast the const away first (the copy reads from this source).
    convolve2_separable.copyToReadOnly(
        constMemPtr,
        reinterpret_cast<CUdeviceptr>(const_cast<aT *>(filter.ptr)),
        fLen * sizeof(aT));
