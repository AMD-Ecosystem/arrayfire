...
__device__ __inline__ int index(int i, int j, int k, int jstride,
                                int kstride) {
    return i + j * jstride + k * kstride;
}

// See convolve1.cuh: a bare complex `*` resolves to HIP's componentwise float2
// operator on the hipRTC device side, so spell the complex product out.
...
#pragma unroll
                for (int fi = 0; fi < fLen0; ++fi) {
                    ...
                    T s_val = shrdMem[index(ci - fi, cj - fj, ck - fk, shrdLen0,
                                            skStride)];
                    accum   = accum + convMul(s_val, f_val);
                }
            }
        }
