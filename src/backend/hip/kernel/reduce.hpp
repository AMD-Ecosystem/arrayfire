...
// hipCUB mirrors CUB component-for-component under namespace hipcub; alias it
// so the unchanged cub:: call sites (WarpReduce/BlockReduce/BlockRadixSort/
// DeviceScan/DeviceReduce) resolve. hipCUB is wave-width-aware (WarpReduce uses
// the device warp size), but any hand-written /32 staging around it is fixed
// separately for wave64.
...
__global__ static void reduce_first_kernel(Param<To> out, CParam<Ti> in,
    ...
    // reduce_first packs one independent reduction per DIMX(=32)-wide row of the
    // block (s_ptr is offset by tidy*DIMX, and tidx==0 of each row writes that
    // row's result). The tree folds within the 32-wide row down to 32 lanes;
    // the final WarpReduce must therefore be a 32-LANE logical warp, NOT the
    // 64-wide device wavefront -- on wave64 a default cub::WarpReduce would
    // reduce across two positionally-packed rows at once (popsift "keep 32-lane
    // sub-groups" axis, the opposite of reduce_all's full-wavefront axis).
    if (DIMX == 256) {
    ...
    typedef cub::WarpReduce<compute_t<To>, 32> WarpReduce;
...
__global__ static void reduce_all_kernel(Param<To> out,
    ...
    // Keep 32-lane logical warps here (NOT the 64-wide device wavefront): a
    // block is (DIMX, THREADS/DIMX) and `cond` is per-tidy-row, so on wave64 a
    // 64-lane wavefront would straddle a cond=true row and a cond=false row, and
    // the `else if(!cond) s_val[warpid]=init` below would RACE the lid==0 write
    // to the same s_val slot. 32-lane groups are uniform in cond (one tidy row)
    // and match the CUDA warp=32 semantics. (popsift keep-32-lane-subgroups axis.)
    const int kRWarp = 32;
    const int nwarps = THREADS_PER_BLOCK / kRWarp;
    ...
    const int warpid = tid / kRWarp;
    const int lid    = tid % kRWarp;
    ...
    // Final fold of the nwarps (<=32) partials by the first 32-lane group.
    if (tid < 32) {
