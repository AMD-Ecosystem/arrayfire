...
static void bcast_dim_launcher(Param<To> out, CParam<To> tmp, Param<int> tlid,
                               const int dim, const uint threads_y,
                               const dim_t blocks_all[4]) {
    // The shared source also defines scanbykey_dim_nonfinal/final, whose bodies
    // use the non-dependent THREADS_X / DIMY; clang/hipRTC parses them even for
    // the bcast instantiation, so both -D must be present here too.
