...
// Explicit signal*filter multiply for the convolution accumulate. A bare
// `cfloat * cfloat` in this templated kernel resolves to HIP's componentwise
// float2 operator on the hipRTC device side (silently wrong: a complex multiply
// is required), so spell the complex product out via .x/.y. Real types keep the
// builtin operator*. Correct on wave32 and wave64 alike.
template<typename Ta, typename Tb>
__device__ inline auto convMul(Ta a, Tb b) -> decltype(a * b) {
    return a * b;
}
__device__ inline cfloat convMul(cfloat a, cfloat b) {
    return cfloat{a.x * b.x - a.y * b.y, a.x * b.y + a.y * b.x};
}
__device__ inline cdouble convMul(cdouble a, cdouble b) {
...
__global__ void convolve1(Param<T> out, CParam<T> signal, int fLen, int nBBS0,
    ...
    if (gx < out.dims[0]) {
        ...
        for (int f = 0; f < fLen; ++f) {
            accum = accum + convMul(shrdMem[lx - f], impulse[f]);
        }
        dst[gx] = (T)accum;
    }
}
