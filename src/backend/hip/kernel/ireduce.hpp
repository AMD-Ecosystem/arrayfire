...
void ireduce_dim_launcher(Param<T> out, uint *olptr, CParam<T> in,
    ...
    // ireduce.cuh defines both ireduceDim and ireduceFirst; their bodies use the
    // non-dependent THREADS_X and THREADS_PER_BLOCK. clang/hipRTC parses both
    // templates regardless of which is instantiated, so both -D must be present
    // for either compile (CUDA's NVRTC tolerated the one-sided define).
    auto ireduceDim = common::getKernel(
        ...
        {{DefineValue(THREADS_X), DefineValue(THREADS_PER_BLOCK)}});
