...
static __global__ void kerTopkDim0(Param<T> ovals, Param<uint> oidxs,
    ...
    // NOTE: keep the radix-sort temp storage and the rearrange buffer in
    // SEPARATE __shared__ allocations (not a union). On ROCm/rocPRIM the
    // BlockRadixSort temp storage aliasing the rearrange buffer corrupts memory
    // (GPU fault); the two are only ~16KB combined, well within the LDS budget.
    __shared__ typename BlockRadixSortT::TempStorage sortmem;
    __shared__ keyValBlocks blkt;
...
#pragma unroll
    for (uint li = 0, i = blockOffset; li < TOPK_IDX_THRD_LOAD;
         i += blockDim.x, li++) {
        if (i < elements) {
            blkt.keys[li * TOPK_THRDS_PER_BLK + threadIdx.x] =
                static_cast<compute_t<T>>(kdata[i]);
            blkt.vals[li * TOPK_THRDS_PER_BLK + threadIdx.x] =
...
#pragma unroll
    for (uint li = 0; li < TOPK_IDX_THRD_LOAD; li++) {
        ...
        keys[li] = blkt.keys[li + (threadIdx.x * TOPK_IDX_THRD_LOAD)];
        vals[li] = blkt.vals[li + (threadIdx.x * TOPK_IDX_THRD_LOAD)];
    }
    __syncthreads();

    if (order & AF_TOPK_MAX) {
        BlockRadixSortT(sortmem)
            .SortDescendingBlockedToStriped(keys, vals);
    } else {
        BlockRadixSortT(sortmem).SortBlockedToStriped(keys, vals);
    }
