...
static void bcast_dim_launcher(Param<To> out, CParam<To> tmp,
    ...
    // scan_dim.cuh also defines the scan_dim template, whose body uses the
    // non-dependent identifier THREADS_X; clang/hipRTC parses it even though only
    // scan_dim_bcast is instantiated, so it must be defined for this compile too.
