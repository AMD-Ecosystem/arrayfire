...
void medfilt1(Param<T> out, CParam<T> in, const af::borderType pad, int w_wid) {
    // medfilt.cuh also defines medfilt2, whose body uses the non-dependent
    // THREADS_X / THREADS_Y; clang/hipRTC parses it even for the medfilt1
    // instantiation, so both -D must be present here too.
    auto medfilt1 =
