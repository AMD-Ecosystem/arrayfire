...
void sparseArithOpCSR(Param<T> out, CParam<T> values, CParam<int> rowIdx,
    ...
    // sparse_arith.cuh defines four kernel templates: csrArith* reference TX/TY,
    // cooArith* reference THREADS. clang/hipRTC does phase-1 lookup on ALL of
    // them regardless of which one this launcher instantiates (the documented
    // multi-template -D gotcha), so every launcher must pass all three defines,
    // not just the ones its own kernel uses.
    auto csrArithDSD = common::getKernel(
        ...
        {{DefineValue(TX), DefineValue(TY), DefineValue(THREADS)}});
