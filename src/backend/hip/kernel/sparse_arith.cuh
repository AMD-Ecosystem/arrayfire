...
// operator() is called from the __global__ csr/coo Arith kernels below. Under
// hipRTC clang treats an unattributed member function as HOST (NVRTC treats the
// whole JIT TU as device), so without __device__ the call from device code
// fails "no matching function" -- the documented arrayfire JIT fault. Tag every
// arith_op::operator() __device__ (kernel-local helper; never called host-side).
template<typename T, af_op_t op>
struct arith_op {
    __device__ T operator()(T v1, T v2) { return T(0); }
};
