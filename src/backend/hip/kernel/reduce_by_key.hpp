...
__global__ void test_needs_reduction(int *needs_another_reduction,
    ...
    int update_key = (k == shfl_down_sync(k, 1)) &&
                     (tid < (n - 1)) && ((threadIdx.x % kWarpSize) < kWarpSize - 1);
    int remaining_updates = any_sync(update_key);

    __syncthreads();

    if (remaining_updates && (threadIdx.x % kWarpSize == 0))
        atomicOr(needs_another_reduction, remaining_updates);

    // check across warp boundaries
    update_key =
        (((threadIdx.x % kWarpSize) == kWarpSize - 1)  // last thread in warp
...
// Max unique keys a single warp can emit == its lane count. The per-warp result
// buffers are sized by this; on wave64 a warp has 64 lanes (up to 64 unique
// keys), so a literal 32 overflows the buffers (OOB) and drops keys. Use
// kWarpSize so the sizing is correct on both wave32 and wave64.
const static int maxResPerWarp = kWarpSize;  // assume dim 0, no NAN values
...
__global__ static void reduce_blocks_by_key(int *reduced_block_sizes,
    ...
    const int laneid = tidx % kWarpSize;

    const int nWarps = DIMX / kWarpSize;
...
#pragma unroll
        for (int off = 1; off < kWarpSize; off <<= 1) {
            v = reduce(v, shfl_down_sync(v, off));
        }
    } else {
...
#pragma unroll
        for (int delta = 1; delta < kWarpSize; delta <<= 1) {
            ...
            update_key =
                eq_check && (laneid < (kWarpSize - delta)) && ((tidx + delta) < n);
    ...
    if (laneid == kWarpSize - 1) { warpReduceSizes[warpid] = unique_id; }
...
#pragma unroll
        for (int offset = 1; offset < kWarpSize; offset <<= 1) {
            // scan over the nWarps per-warp sizes held in the first nWarps
            // lanes; the wrapper uses the 64-bit all-lanes FULL_MASK (wave64).
            char y = shfl_up_sync(warpSzScan, offset);
