...
static void bcast_first_launcher(Param<To> out, CParam<To> tmp,
    ...
    // scan_first.cuh also defines the scan_first template, whose body uses the
    // non-dependent identifier THREADS_PER_BLOCK. NVRTC only instantiates the
    // requested scan_first_bcast and tolerates the unused template; clang/hipRTC
    // does phase-1 lookup on it regardless, so THREADS_PER_BLOCK must be defined
    // for this compile too (CUDA never needed the -D here).
