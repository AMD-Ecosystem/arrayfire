...
void floodFill(Param<T> out, CParam<T> image, CParam<uint> seedsx,
    ...
    // flood_fill.cuh also defines floodStep, whose body uses the non-dependent
    // THREADS_X / THREADS_Y; clang/hipRTC parses it for every instantiation, so
    // all three launchers of this source must pass both -D.
    auto initSeeds =
