...
void nonMaximal(float* x_out, float* y_out, float* resp_out, unsigned* count,
    ...
    // susan.cuh also defines the susan kernel, whose body uses the non-dependent
    // BLOCK_X / BLOCK_Y; clang/hipRTC parses it for the nonMax instantiation too,
    // so both -D must be present here.
    auto nonMax =
