...
// boxMuller's compute type Tc can be __half while the output type Td is
// common::half. On NVIDIA common::half's native storage IS __half so
// static_cast<Td>(__half) works; on HIP the native storage is uint16_t and
// there is no half(__half) ctor, so route __half -> float -> Td (common::half
// has a half(float) ctor). For every other (Td,Tc) the cast is direct, so f32/
// f64 precision is untouched.
template<typename Td, typename Tc>
__device__ static Td boxMullerCast(Tc v) {
    return static_cast<Td>(v);
}
template<>
__device__ inline common::half boxMullerCast<common::half, __half>(__half v) {
    return static_cast<common::half>(__half2float(v));
}
...
__device__ static void boxMullerTransform(Td *const out1, Td *const out2,
    ...
    sincospi(r1, &s, &c);
    *out1 = boxMullerCast<Td>(r * s);
    *out2 = boxMullerCast<Td>(r * c);
}
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ < 530
template<>
