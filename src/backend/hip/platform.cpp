...
// The sparse subsystem is implemented on hipSPARSE. Unlike the CUDA backend,
// which loads cuSPARSE through a dlopen plugin (cusparseModule), the HIP build
// links hipSPARSE directly, so sparseHandle() below creates the handle eagerly.
#include <cuda_fp16.h>  // __half for the af::array::device<__half> interop (shim -> hip_fp16.h)
...
BlasHandleRAII *cublasManager(const int deviceId) {
    thread_local BlasHandleRAII handles[DeviceManager::MAX_DEVICES];
    thread_local once_flag initFlags[DeviceManager::MAX_DEVICES];

    call_once(initFlags[deviceId], [&] {
        CUBLAS_CHECK((hipblasStatus_t)handles[deviceId].create());
...
SolveHandleRAII *cusolverManager(const int deviceId) {
    thread_local SolveHandleRAII handles[DeviceManager::MAX_DEVICES];
    thread_local once_flag initFlags[DeviceManager::MAX_DEVICES];
    call_once(initFlags[deviceId], [&] {
        ...
        // call outside of call_once scope.
        CUSOLVER_CHECK(
            hipsolverDnSetStream(handles[deviceId], getStream(deviceId)));
    });
...
SparseHandleRAII *cusparseManager(const int deviceId) {
    thread_local SparseHandleRAII handles[DeviceManager::MAX_DEVICES];
    thread_local once_flag initFlags[DeviceManager::MAX_DEVICES];
    call_once(initFlags[deviceId], [&] {
        handles[deviceId].create();
        hipsparseSetStream(handles[deviceId], getStream(deviceId));
    });
    return &handles[deviceId];
}
...
string getCUDARuntimeVersion() noexcept {
    ...
    if (cudaSuccess == cudaRuntimeGetVersion(&runtime)) {
        ...
        // HIP has no CUDA_VERSION; report the HIP runtime version it was built
        // against (HIP_VERSION_MAJOR/MINOR from hip/hip_version.h).
        return int_version_to_string(HIP_VERSION_MAJOR * 1000 +
                                     HIP_VERSION_MINOR * 10);
    }
}
