...
#include <hipblas/hipblas.h>  // needed for af/cuda.h
#include <driver.h>
...
struct cuNVRTCcompute {
    /// The CUDA Toolkit version returned by hipRuntimeGetVersion
    int cudaVersion;
...
struct ToolkitDriverVersions {
    /// The CUDA Toolkit version returned by hipDriverGetVersion or
...
static inline int compute2cores(unsigned major, unsigned minor) {
    ...
    // The NVIDIA compute-capability table does not describe AMD CUs (and gfx90a
    // major=9 would alias the sm_90 entry). The result only feeds an AF_TRACE
    // line and a same-arch flops-based device sort, so a neutral per-CU lane
    // count keeps the GFLOPs estimate representative without an NVIDIA mapping.
    (void)major;
    (void)minor;
    return 64;
#else
...
void checkAndSetDevMaxCompute(pair<int, int> &computeCapability) {
    ...
    int rtCudaVer        = 0;
    CUDA_CHECK(hipRuntimeGetVersion(&rtCudaVer));
...
bool DeviceManager::checkGraphicsInteropCapability() {
    // Headless ROCm build: Forge graphics and OpenGL interop are disabled, so
    // hipGLGetDevices is unavailable. Report no GL interop capability. (The
    // NVIDIA backend keeps the cudaGLGetDevices probe.)
    return false;
}
...
/// This function initializes and deletes a hiprtcProgram object. There seems to
...
void initNvrtc() {
    hiprtcProgram prog;
    hiprtcCreateProgram(&prog, " ", "dummy", 0, nullptr, nullptr);
...
DeviceManager::DeviceManager()
    ...
    try {
        ...
        int cudaRtVer = 0;
        CUDA_CHECK(hipRuntimeGetVersion(&cudaRtVer));
        int cudaMajorVer = cudaRtVer / 1000;

        for (int i = 0; i < nDevices; i++) {
            cudaDevice_t dev{};
            CUDA_CHECK(hipGetDeviceProperties(&dev.prop, i));
    ...
    for (int i = 0; i < nDevices; i++) {
        for (int j = 0; j < nDevices; j++) {
            if (i != j) {
                int can_access_peer;
                CUDA_CHECK(hipDeviceCanAccessPeer(&can_access_peer, i, j));
                if (can_access_peer) {
    ...
    for (int i = 0; i < MAX_DEVICES; i++) {
        streams[i] = static_cast<hipStream_t>(0);
        if (i < nDevices) {
            auto prop =
...
int DeviceManager::setActiveDevice(int device, int nId) {
    ...
    hipError_t err = hipSetDevice(nId);
    ...
    while (true) {
        ...
        if (err != hipErrorContextAlreadyInUse) {
            CUDA_CHECK(err);
            tlocalActiveDeviceId() = device;
            return old;
        }
        hipGetLastError();  // Reset error stack
#ifndef NDEBUG
        getLogger()->warn(
