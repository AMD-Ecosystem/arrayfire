...
struct geqrf_func_def_t {
    using geqrf_func_def = hipsolverStatus_t (*)(hipsolverHandle_t, int, int,
                                                T *, int, T *, T *, int, int *);
};

template<typename T>
struct geqrf_buf_func_def_t {
    using geqrf_buf_func_def = hipsolverStatus_t (*)(hipsolverHandle_t, int,
                                                    int, T *, int, int *);
};

template<typename T>
struct mqr_func_def_t {
    using mqr_func_def = hipsolverStatus_t (*)(hipsolverHandle_t,
                                              hipblasSideMode_t,
...
struct mqr_buf_func_def_t {
    using mqr_buf_func_def = hipsolverStatus_t (*)(hipsolverHandle_t,
                                                  hipblasSideMode_t,
                                                  hipblasOperation_t, int, int, int,
                                                  const T *, int, const T *, T *,
                                                  int, int *);
...
void qr(Array<T> &q, Array<T> &r, Array<T> &t, const Array<T> &in) {
    ...
    CUSOLVER_CHECK(mqr_buf_func<T>()(
        solverDnHandle(), HIPBLAS_SIDE_LEFT, HIPBLAS_OP_N, q.dims()[0],
